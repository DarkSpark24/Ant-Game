...
- 蚂蚁具有年龄。年龄等于**当前回合数 - 生成回合数**。当年龄大于64时，蚂蚁死亡/消失。
- 蚂蚁具有两个属性：
  - **兵种(kind)**：**普通蚂蚁**、**战斗蚂蚁**。
...
- **降级、拆除返还**：降级、拆除防御塔会返还 $90\%$ 的建造花销，等级2降级为等级1返还54金；拆除等级1时返还 $\lfloor 15\times 2^i \times 90\% \rfloor$ 金，其中$i$为**拆除之后**己方防御塔的数量。
...
- **防御塔数据列表**
  ...
  | 11     | Heavy+             | 35   | 2    | 2    | 单体攻击 | 1 |
  | 12     | Ice                | 15   | 2    | 1    | 单体攻击；命中的蚂蚁若非免控，则冻结1回合，解冻后转为**随机型** | 1 |
  | 13     | Cannon / Bewitch   | 15   | 3    | 2    | 单体攻击；命中的蚂蚁若非免控，则转为**蛊惑型**。若其在己方半场，目标为敌方基地；若已在敌方半场，目标为所属方己方半场内的随机合法位置 | 1 |
  | 2      | Quick              | 6    | 1    | 2    | 单体攻击 | 0 |
  | 21     | Quick+             | 8    | 0.5  | 2    | 单体攻击 | 2 |
  | 22     | Double             | 7    | 1    | 3    | 最多攻击优先级前二的目标 | 2 |
  | 23     | Sniper             | 10   | 2    | 4    | 单体攻击 | 2 |
  | 3      | Mortar             | 10   | 4    | 2    | 以目标点为中心、范围1的AOE | 0 |
  | 31     | Mortar+            | 20   | 4    | 3    | 以目标点为中心、范围1的AOE | 3 |
  | 32     | Pulse              | 12   | 3    | 1    | 同时攻击自身范围内所有敌方蚂蚁，并将命中的蚂蚁转为**随机型** | 3 |
  | 33     | Missile            | 25   | 6    | 4    | 以目标点为中心、范围2的AOE | 3 |
...
- **战斗蚂蚁**：更偏向寻塔拆塔，拆塔伤害固定为 `5`，最大生命固定为 `30`，出生时自带 `3` 层**紧急回避**；若当前生命值低于自身最大生命值一半，则攻击塔时改为自爆：对目标塔及其距离 `1` 内的敌方防御塔各造成 `10` 点伤害，然后自身死亡。基地与产蚁塔会以较高概率自然生成为战斗蚂蚁，`ProducerSiege` 还会额外生成战斗蚂蚁。
...
2. 回合结算
   ...
   5. 如果蚂蚁的年龄大于最大年龄，即**当前回合数-蚂蚁生成回合数大于64**，标记为老死。
   6. 按照生成顺序结算蚂蚁移动。
      1. 已死亡的蚂蚁不可以移动。
