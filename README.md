...
- 蚂蚁具有年龄。年龄等于**当前回合数 - 生成回合数**。当年龄大于32时，蚂蚁死亡/消失。
- 蚂蚁具有两个属性：
  - **兵种(kind)**：**普通蚂蚁**、**战斗蚂蚁**。
...
- **降级、拆除返还**：降级、拆除防御塔会返还 $80\%$ 的建造花销，等级2降级为等级1返还48金，拆除等级1返还$12\times 2^i$金，其中$i$为**拆除之后**己方防御塔的数量。
...
- **防御塔数据列表**
  ...
  | ------ | ------------------ | ---- | ---- | ---- | ---------------- | ---------- |
  | 0      | Basic              | 5    | 2    | 2    | 单体攻击 | 无 |
  | 1      | Heavy              | 20   | 2    | 2    | 单体攻击 | 0 |
  | 11     | Heavy+             | 35   | 2    | 3    | 单体攻击 | 1 |
  | 12     | Ice                | 15   | 2    | 2    | 单体攻击；命中的蚂蚁若非免控，则冻结1回合，解冻后转为**随机型** | 1 |
  | 13     | Cannon / Bewitch   | 10   | 3    | 3    | 单体攻击；命中的蚂蚁若非免控，则转为**蛊惑型**。若其在己方半场，目标为敌方基地；若已在敌方半场，目标为所属方己方半场内的随机合法位置 | 1 |
  | 2      | Quick              | 6    | 1    | 3    | 单体攻击 | 0 |
  | 21     | Quick+             | 8    | 0.5  | 3    | 单体攻击 | 2 |
  | 22     | Double             | 7    | 1    | 4    | 最多攻击优先级前二的目标 | 2 |
  | 23     | Sniper             | 15   | 2    | 6    | 单体攻击 | 2 |
  | 3      | Mortar             | 16   | 4    | 3    | 以目标点为中心、范围1的AOE | 0 |
  | 31     | Mortar+            | 35   | 4    | 4    | 以目标点为中心、范围1的AOE | 3 |
  | 32     | Pulse              | 12   | 3    | 2    | 同时攻击自身范围内所有敌方蚂蚁，并将命中的蚂蚁转为**随机型** | 3 |
  | 33     | Missile            | 45   | 6    | 5    | 以目标点为中心、范围2的AOE | 3 |
  | 4      | Producer           | 0    | 4    | 0    | 不主动攻击；周期性在相邻路径格产出普通蚂蚁 | 0 |
  | 41     | Producer+       | 0    | 2    | 0    | 不主动攻击；更快地产出普通蚂蚁 | 4 |
  | 42     | Siege      | 0    | 4    | 0    | 不主动攻击；正常产出普通蚂蚁，并以25%概率额外产出1只**战斗蚂蚁** | 4 |
  | 43     | Medic      | 0    | 4    | 4    | 不主动攻击；正常产出普通蚂蚁，并为范围4内最靠前的己方蚂蚁回复4生命并附加1层**紧急回避** | 4 |

  注：

  - 当前所有防御塔的最大血量均为 `10`。 (TODO: 高级防御塔为15)
  - 生产塔分支（`4 / 41 / 42 / 43`）不会主动攻击。
  - 所有改变蚂蚁行为的防御塔，若命中的蚂蚁已是**免控型**，则控制效果无效。
  - 蚂蚁可以对相邻敌方塔发起攻击；这会被视为“向塔移动”的决策，但蚂蚁本回合**不会真正进入塔格**，只结算对塔伤害。
  - 普通蚂蚁拆塔伤害按等级分别为 `1 / 2 / 4`；战斗蚂蚁固定为 `3`。若战斗蚂蚁攻击的塔当前血量小于 `5`，则与塔同归于尽。
...
- 若选择的是相邻敌方塔所在格，则本回合结算为**攻击塔**：蚂蚁实际位置不变化，只扣除该塔血量。
...
- **战斗蚂蚁**：更偏向寻塔拆塔，拆塔伤害固定为 `3`，出生时自带 `2` 层**紧急回避**；若攻击的塔当前血量小于 `5`，则与塔同归于尽。当前只有 `ProducerSiege` 会额外生成战斗蚂蚁。

#### 行为与生成概率

基地与产蚁塔生成的新蚂蚁，其行为按以下概率随机确定：
...
- **蛊惑型**：目标不再是敌方基地，而是由 `Cannon / Bewitch` 指定的内部目标格；其余仍使用风险感知评分，但更偏向朝该目标推进，并允许回头。若提前抵达目标格，或持续5回合结束，则退化为**默认型**。
...
- `control` 估计该格被 `Ice / Cannon / Pulse` 命中的控制风险。
...
2. 回合结算
   ...
   3. 按照建造顺序结算防御塔攻击。攻击时首先判断塔是否处于EMP干扰区域内（若是，则该防御塔无法攻击）；然后执行伤害和附加效果。生产塔分支不会主动攻击。
...
第二行一个整数`N_1`，代表场上总防御塔数量。接下来`N_1`行，每行**7**个整数，分别为`id player x y type cd hp`，即每个防御塔的ID、归属、坐标、类型、攻击CD、当前血量。
